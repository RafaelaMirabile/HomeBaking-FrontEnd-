...
function postTransaction(body: Required<TransactionsInputs>) {
  const config = {
    headers: {
      Authorization: `Bearer ${userToken}`
    }
...
function deleteTransaction(transactionId: string) {
  ...
  const promise = axios.delete(`${URL_BASE}/funds/${transactionId}`, config);
  return promise;
}


export { postRegistration, getTransactions, postLogin, postTransaction, deleteTransaction }
